...
- **Core goal:** Detect suspicious traffic in an SDN lab using an FT-Transformer model, visualize the network state in near real-time, and apply operator-driven or automatic mitigations through OpenFlow/OVS.
...
Mininet hosts/switches
        |
        v
Ryu controller (OpenFlow 1.3) + FT-Transformer inference
        |
...
- **ML:** PyTorch FT-Transformer + sklearn/joblib preprocessing artifacts
...
3. FT-Transformer predicts label/confidence.
